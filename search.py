...
def whateverFirstSearch(problem, frontier):
    ...
    """
    "*** YOUR CODE HERE ***"
    # Each frontier entry is a search node: (state, actions to reach it, path cost)
    ...
    while not frontier.isEmpty():
        ...
        # Graph search: never expand the same state twice
        if state in explored:
            continue

        if problem.isGoalState(state):
            return actions, cost, gets, puts

        explored.add(state)

        for successor, action, stepCost in problem.getSuccessors(state):
            if successor not in explored:
                frontier.push((successor, actions + [action], cost + stepCost))
                puts += 1

    return None, None, gets, puts
...
def depthFirstSearchStats(problem):
    ...
    """
    # A LIFO stack expands the deepest node first
    return whateverFirstSearch(problem, util.Stack())
...
def breadthFirstSearchStats(problem):
    ...
    "*** YOUR CODE HERE ***"
    # A FIFO queue expands the shallowest node first
    return whateverFirstSearch(problem, util.Queue())
